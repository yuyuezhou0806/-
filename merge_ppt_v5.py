"""
Merge PowerPoint presentations using proper XML manipulation.
"""
import zipfile
import tempfile
import shutil
...
def merge_presentations(output_path, *input_paths):
    ...
    # Track existing media files and their content hashes
    ...
    # Parse presentation.xml.rels using lxml
    ...
    # Media mapping: (input_idx, orig_name) -> merged_name
    media_map = {}
    ...
    for input_idx, input_path in enumerate(input_paths[1:], 1):
        ...
        if os.path.exists(add_media):
            ...
            for media_file in sorted(os.listdir(add_media)):
                ...
                # Check if identical media already exists
                found = False
                ...
                if not found:
                    # Use original name or find unique name
                    dst_name = media_file
                    counter = 1
        ...
        # Build rId -> target mapping for slides
        slide_targets = {}
