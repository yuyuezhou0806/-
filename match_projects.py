...
for s_client, s_proj in screenshot_data:
    ...
    matched = best_score >= 0.90
    results.append({
        "截图工程名称": s_proj,
        "截图客户名称": s_client,
        "匹配到的工程名称": best_match["proj"] if matched else "",
        "匹配到的客户名称": best_match["client"] if matched else "",
        "合同编号备注": best_match["note"] if matched else "",
        "归档状态": best_match["archive"] if matched else "未匹配",
        "相似度": round(best_score, 4),
    })
    print(f"{'[MATCH]' if matched else '[SKIP]'} {s_proj[:40]}... -> {best_score:.2%}")
...
for r in results:
    row_data = [r["截图工程名称"], r["截图客户名称"], r["合同编号备注"], r["归档状态"]]
...
# Print summary
matched_count = sum(1 for r in results if r["相似度"] >= 0.90)
