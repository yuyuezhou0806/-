...
add_image_slide(slide, f"{IMG_DIR}\\3657e406-70e0-42f7-aa1a-68f029125aa5.png",
                Inches(6.2), Inches(1.0), Inches(6.8), Inches(6.2))

# ========== 第3页：进入商机 ==========
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_top_bar(slide)
add_title(slide, "第二步：进入商机")
add_bullet_text(slide, [
    "1. 左侧菜单展开「商机」",
    "2. 点击「商机」进入商机列表",
    "3. 查看已有商机记录"
], Inches(0.4), Inches(1.1), Inches(5.5), Inches(3))
add_image_slide(slide, f"{IMG_DIR}\\0ea2e5e1-05e8-4da2-a72c-3d4f93aaffc9.png",
                Inches(6.2), Inches(1.0), Inches(6.8), Inches(6.2))

# ========== 第4页：新建商机 ==========
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_top_bar(slide)
add_title(slide, "第三步：新建商机")
add_bullet_text(slide, [
    "1. 在商机列表页面",
    "2. 点击右上角「新建」按钮",
    "3. 系统弹出商机录入表单"
], Inches(0.4), Inches(1.1), Inches(5.5), Inches(3))
add_image_slide(slide, f"{IMG_DIR}\\11404e15-ef8d-4261-92d8-41359888cb67.png",
                Inches(6.2), Inches(1.0), Inches(6.8), Inches(6.2))
...
add_title(slide, "第四步：填写输入框（红色边框）", color=RGBColor(0xD4, 0x4B, 0x4B))
add_bullet_text(slide, [
    "商机名称：填写项目全称",
    "      例：嘉松南路城桂路交叉口新建工程",
    "",
...
add_title(slide, "第五步：选择基本信息（黄色边框）", color=RGBColor(0xC7, 0x94, 0x00))
add_bullet_text(slide, [
    "子公司：选择承接主体  例：中测行",
    "业务经理：选择负责人  例：卫天勇",
    "业态：选择业务类型  例：工程检测",
...
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_top_bar(slide, RGBColor(0xC7, 0x94, 0x00))
add_title(slide, "第六步：选择日期与其他字段（黄色边框）", color=RGBColor(0xC7, 0x94, 0x00))
add_bullet_text(slide, [
    "📅 开标日期：项目公开招标日期",
    "      例：2026-05-19",
    "📅 预计定标日期：预计确定中标方日期",
...
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_top_bar(slide)
add_title(slide, "第七步：填写客户信息与参与人员")
add_bullet_text(slide, [
    "客户名称：从系统客户库中选择",
    "      例：上海廷瑞建设工程有限公司",
    "      （绿色特殊框，关联CRM客户主数据）",
